...
function drawQueueTable(aoCssQueue) {
  ...
  jQuery.each(aoCssQueue, function(path, keys) {
    ...
    } else if ((keys.jqstat === 'JOB_DONE' || keys.jqstat === 'JOB_FAILED' || keys.jqstat === 'STATUS_JOB_BAD' || keys.jqstat === 'INVALID_JWT_TOKEN' || keys.jqstat === 'NO_RESPONSE') && (keys.jrstat !== 'GOOD' || (keys.jvstat !== 'GOOD' || keys.jvstat !== 'WARN' || keys.jvstat !== 'BAD'))) {
